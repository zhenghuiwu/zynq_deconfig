...
int image_sel(void){
	...
	imagesel = env_get("imagesel");
	if(imagesel==NULL)
	{
		printf("[INFO] get imagesel NULL");
		return 1;
	};

	printf("[INFO] Current Boot Image Is %s\n\r",imagesel);

	if (!strcmp(imagesel, "image-a")){
		printf("[INFO] Next Boot Image Is image-b\n\r");
	    env_set("imagesel", "image-b");
		env_save();
		return 0;
	}
    else if(!strcmp(imagesel, "image-b")){
		printf("[INFO] Next Boot Image is image-a\n\r");
        env_set("imagesel", "image-a");
		env_save();
		return 0;
    }
    else{
		printf("[INFO] No imagesel in env!\n\r");
		return 1;
    }
